...
- Prior pipeline artifacts -- any existing `Shipyard/` outputs from other skills
...
generated_from: Shipyard/security-engineer/threat-model/stride-analysis.md
controls:
  - id: SR-001
...
- A `required` control may only become `accepted` via a logged "accepted with justification" override receipt (see `Shipyard/.protocols/conflict-resolution.md` / `receipt-protocol.md`) — never silently dropped.
...
Write all outputs to `Shipyard/security-engineer/threat-model/`:
